...
target_models: ["Claude 4+", "GPT-4+", "Gemini 2.5+"]
...
## Context Mapper CML Generation

### context-map.cml
```
ContextMap [SystemName] {
  contains [BoundedContext1], [BoundedContext2], [BoundedContext3]
  
  [BoundedContext1] [Pattern] [BoundedContext2] {
    implementationTechnology "REST"
    downstreamRights INFLUENCER
    exposedAggregates [AggregateA], [AggregateB]
  }
  
  [BoundedContext2] [ACL] -> [BoundedContext3]
}

BoundedContext [BoundedContext1] {
  type FEATURE
  domainVisionStatement "Purpose statement"
...
```bash
#!/bin/bash
# Validate DDD model against checklist

MODEL=".specify/ddd/model.json"
CHECKLIST=".specify/ddd/validation-checklist.md"

# Run strategic validation
# Run tactical validation
# Run integration validation
# Update checklist with results

echo "Validation complete. See $CHECKLIST for details."
```
...
## Example 1: Requirements Mode - E-Commerce Marketplace
...
- `catalog-context.cml`, `ordering-context.cml`, etc.
...
## Example 2: Repository Mode - Microservices Monorepo
...
1. **Detected Contexts** (by service boundaries):
...
3. **Events Discovered** (from shared-lib/events):
...
### Generated Artifacts
- `model.json` with 3 bounded contexts, 4 aggregates, 8 domain events
